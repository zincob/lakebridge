...
- The Remorph tool should be configured through CLI to create the remorph catalog (the name of the catalog can be customized during installation)
...
| **primary_key**                | `<optional>` The primary key columns to be used as an array. This will initiate the remorph reconcile job to run the comparison including primary keys. If unspecified runs a row level comparison.|
| **source_filters**    | `<optional>` The filters to be applied on the source table. This will initiate the remorph reconcile job to run the comparison including filters. |
| **databricks_filters**| `<optional>` The filters to be applied on the databricks table. This will initiate the remorph reconcile job to run the comparison including filters. |
...
| **select_columns**    | `<optional>` The columns to be selected from the source table. This will initiate the remorph reconcile job to run the comparison including selected columns. |
| **drop_columns**      | `<optional>` The columns to be dropped from the source table. This will initiate the remorph reconcile job to run the comparison including dropped columns. |
...
[Link to the notebook](/remorph_reconciliation.zip) - Unzip the downloaded file and upload the notebook file to your Databricks workspace.
